...
- **JSG properties**: three property macros on `#[jsg_resource]` impl blocks — `#[jsg_prototype_property]` (prototype accessor, not enumerable, equivalent to `JSG_PROTOTYPE_PROPERTY`), `#[jsg_instance_property]` (own-property accessor, enumerable, equivalent to `JSG_INSTANCE_PROPERTY`; add `lazy` flag for `JSG_LAZY_INSTANCE_PROPERTY`), and `#[jsg_inspect_property]` (registered under a unique symbol, invisible to normal enumeration and string-key lookup, surfaced by `node:util` `inspect()`, equivalent to `JSG_INSPECT_PROPERTY`); setter auto-detected from `set_` prefix; read-only when no setter present; all three accept `name = "..."` to override the JS property name; getter/setter `.length` and `.name` are set correctly when `spec_compliant_property_attributes` compat flag is enabled
