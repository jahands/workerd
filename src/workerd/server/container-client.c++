...
kj::Promise<void> ContainerClient::start(StartContext context) {
  ...
  if (params.hasContainerSnapshot()) {
    auto snapshot = params.getContainerSnapshot();
    auto snapshotId = parseSnapshotId(snapshot.getId());
...
kj::Promise<void> ContainerClient::snapshotContainer(SnapshotContainerContext context) {
  ...
  JSG_REQUIRE(!params.getMemory(), Error,
      "snapshotContainer({ memory: true }) is not supported for local Docker containers.");
