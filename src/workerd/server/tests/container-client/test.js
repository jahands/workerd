...
export class DurableObjectExample extends DurableObject {
  ...
  async restoreTransferredContainerSnapshot(snapshot) {
    assert.ok(snapshot.id, 'snapshot must have a non-empty id');
    assert.ok(snapshot.size > 0, 'snapshot must have a positive size');
    assert.strictEqual(snapshot.memory, false);
  ...
  async testContainerSnapshotMemoryUnsupported() {
    const container = this.ctx.container;
    if (container.running) {
      const monitor = container.monitor().catch((_err) => {});
      await container.destroy();
    ...
    await assert.rejects(
      () => container.snapshotContainer({ memory: true }),
      (err) => {
        assert.match(err.message, /not supported for local Docker containers/);
        return true;
      }
    );
  ...
  async testContainerSnapshotWithDirectoryOverlay() {
    ...
    await container
      .getTcpPort(8080)
      .fetch(
        'http://foo/write-file?path=/tmp/overlay-source/from-overlay.txt',
        {
          method: 'POST',
    ...
    container.start({
      ...
      containerSnapshot,
      directorySnapshots: [
        { snapshot: directorySnapshot, mountPoint: '/app/overlay' },
      ],
    });
    const monitor2 = container.monitor().catch((_err) => {});
    await this.waitUntilContainerIsHealthy();
    ...
    const overlayResp = await container
      .getTcpPort(8080)
      .fetch('http://foo/read-file?path=/app/overlay/from-overlay.txt', {
        signal: AbortSignal.timeout(DEFAULT_TIMEOUT_DURATION),
      });
...
// Test that local dev rejects memory snapshots for full container snapshots.
export const testContainerSnapshotMemoryUnsupported = {
  async test(_ctrl, env) {
    const id = env.MY_CONTAINER.idFromName(
      getRandomDurableObjectName('testContainerSnapshotMemoryUnsupported')
    );
    const stub = env.MY_CONTAINER.get(id);
    await stub.testContainerSnapshotMemoryUnsupported();
  },
};
